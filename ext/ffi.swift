...
// Provides C-compatible interface for Crystal
//
// All @_cdecl functions use C calling convention and are safe to call from Crystal.
...
/// Tool callback type - Crystal side implements this
/// Parameters: userData, toolName, argumentsJson
/// Returns: resultJson (Crystal-allocated, caller must free with free())
...
/// Tool definition parsed from Crystal JSON
...
final class ToolDispatcher: @unchecked Sendable {
    ...
    func callTool(name: String, argumentsJson: String) throws -> String {
        ...
        // Free the Crystal-allocated string
        free(resultPtr)
...
/// invoke Crystal-defined tools dynamically. The model calls `invoke_tool` with the
/// tool name and JSON arguments, which dispatches to the appropriate Crystal callback.
