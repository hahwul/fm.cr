# AGENTS.md - AI Agent Instructions for the fm.cr Documentation Site

This document is for AI agents editing the fm.cr documentation site under `docs/`.

## Project Overview

This is the documentation companion to [fm.cr](https://github.com/hahwul/fm.cr), Crystal bindings for Apple's FoundationModels framework (on-device AI via Apple Intelligence). The site is a static site built with [Hwaro](https://github.com/hahwul/hwaro) (Crinja/Jinja2 templates).
...
- fm.cr requires **macOS 26+ (Tahoe)** with Apple Intelligence enabled - keep that requirement visible in getting-started material.
- Code samples must be valid Crystal that runs against the latest fm.cr - copy from the repo's `examples/` directory when in doubt.
...
3. fm.cr is the only site with per-site slot partials in use: `templates/partials/brand.html` (sidebar logo) and `templates/partials/icons.html` (favicons). Keep them when syncing template updates from sibling sites.
